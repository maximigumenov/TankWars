...
namespace Gaia
{
    ...
    public class StamperEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                seaLevel = EditorGUILayout.Slider(GetLabel("Sea Level"), m_stamper.m_resources.m_seaLevel, 0f, m_stamper.m_resources.m_terrainHeight);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (m_stamper.m_resources != null && m_stamper.m_resources.m_seaLevel != seaLevel)
                {
                    m_stamper.m_resources.ChangeSeaLevel(m_stamper.m_resources.m_seaLevel, seaLevel);
                    EditorUtility.SetDirty(m_stamper.m_resources);
                }
                //m_stamper.m_gizmoColour = gizmoColour;
        ...
        static Dictionary<string, string> m_tooltips = new Dictionary<string,string>
        {
            ...
            { "Area Mask", "Masks the effect of the stamp using the strength of the texture provided. A value of 1 in the texture is means apply full effect, a value of 0 in the texture means apply no effect. This works in the same was as a greyscale heightmap works. If using a terrain texture, then paint on the terrain with the selected texture, and the painted area will be used as the mask."},
