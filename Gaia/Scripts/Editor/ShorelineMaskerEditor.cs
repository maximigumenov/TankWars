        ...
        public void CreateMask()
        {
            ...
            for (int x = 0; x < hm.Width(); x++)
            {
                for (int z = 0; z < hm.Depth(); z++)
                {
                    ...
                    else if (Gaia.Utils.Math_ApproximatelyEqual(testHeight, nrmSeaLevel, 0.001f))
                    {
            ...
            //Now save the mask
            string path = string.Format("{0}-Mask-{1:yyyyMMdd-HHmmss}", Terrain.activeTerrain.name, DateTime.Now);
